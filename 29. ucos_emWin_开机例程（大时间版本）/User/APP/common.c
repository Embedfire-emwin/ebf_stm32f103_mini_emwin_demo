...
static const GUI_WIDGET_CREATE_INFO _aDialogCreateError[] = {
  { FRAMEWIN_CreateIndirect, "Error",  ERROR_FRAMEWIN_0,  0,   0,   200, 160, 0, 0x0, 0 },
  { BUTTON_CreateIndirect,   "CLOSE",  ERROR_BUTTON_0,    20,  80,  60,  30,  0, 0x0, 0 },
  { BUTTON_CreateIndirect,   "Reboot", ERROR_BUTTON_1,    110, 80,  60,  30,  0, 0x0, 0 },
...
static void _cbDialogError(WM_MESSAGE * pMsg) {
    ...
    if(NCode==WM_NOTIFICATION_RELEASED)
    {
      switch(Id)
      {
        ...
        case ERROR_BUTTON_1: // Notifications sent by 'Reboot'
          Soft_Reset();//WM_DeleteWindow(WM_GetParent(pMsg->hWin));
          break;
      }
    }    
    break;
  ...
  case WM_PAINT:
      ...
      GUI_DispStringHCenterAt(ErrorStr[0],90,5);      
      GUI_DispStringHCenterAt(ErrorStr[1],90,35);
   break;
  // USER END
  default:
    WM_DefaultProc(pMsg);
...
void ErrorDialog(WM_HWIN hParent,char *string1,char *string2)
{
  ...
  ErrorStr[0]=string1;
  ErrorStr[1]=string2;
  hWin=GUI_CreateDialogBox(_aDialogCreateError,GUI_COUNTOF(_aDialogCreateError),_cbDialogError,hParent,20,70);
  WM_MakeModal(hWin);
