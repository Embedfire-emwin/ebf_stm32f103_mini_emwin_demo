...
static void _WaitForPressedState(int Pressed) {
  ...
  do {
    ...
    if (State.Pressed == Pressed) {
      int TimeStart = GUI_X_GetTime();
      do {
        GUI_TOUCH_GetState(&State);
        GUI_Delay(1);
        if (State.Pressed != Pressed) {
          break;
        } else if ((GUI_X_GetTime() - 50) > TimeStart) {
          return;
        }
      } while (1);
    }
...
static void _GetPhysValues(int LogX, int LogY, int * pPhysX, int * pPhysY, const char * pString) {
  ...
  //
	if(LogY < LCD_GetYSize()/2)
	{
		Rect.y0 = LogY+10 - FontSizeY;
		Rect.y1 = LogY+10 + FontSizeY;
	}else
	{
		Rect.y0 = LogY-10 - FontSizeY;
		Rect.y1 = LogY-10 + FontSizeY;
...
void TOUCH_Calibrate(void) 
{
	...
	aCal[0]=5;
	aCal[1]=5;
	aCal[4]=LCD_GetXSize() - 5;
	aCal[5]=LCD_GetYSize() - 5;
	...
	if((aCal[2]<600&&aCal[2]>200)&&(aCal[3]<3750&&aCal[3]>3350)&&(aCal[6]<3800&&aCal[6]>3400)&&(aCal[7]<550&&aCal[7]>150))
	{
		...
		SPI_FLASH_BufferWrite((void *)aCal,CALADD+2, sizeof(aCal));	
			...
			SPI_FLASH_BufferRead((void *)aCal,CALADD+2, sizeof(aCal));
	for( i=0; i<8; ++i)
        printf("readCal[%d]=%d ",i,aCal[i]);
	printf("\r\n");
