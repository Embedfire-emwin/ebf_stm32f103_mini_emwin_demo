...
-应用界面的创建在PhoneDLG.c文件中，点击图标后执行FUN_ICON4Clicked函数创建界面

-创建界面后在界面的回调函数_cbDialogPhone中执行各种响应操作


【 ！】实验操作：
连接好配套的3.2_2.8寸液晶屏，

连接好GSM模块,并给模块插入SIM卡、耳机、麦克风：
	PA2<--->GSM_TXD
	PA3<--->GSM_RXD
	GND<--->GND

GSM模块需要使用DC电源供电。
给开发板上电并复位，点击电话应用，可拨打电话
...
-由于STM32的FLASH空间不足，本程序把appicon.c中的图标格式更换成了如下格式：
